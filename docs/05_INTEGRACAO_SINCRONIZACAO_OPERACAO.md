...
- [ ] Conta **Resend** com domínio `plexalabs.com` verificado (DKIM + SPF). API key com escopo "Sending access".
- [ ] Conta Supabase com plano Pro ativo (organização da Plexalabs).
- [ ] Acesso ao DNS na **GoDaddy** (zona `plexalabs.com`) para criar CNAME `caixa-boti` → `controle-caixa.pages.dev` quando UAT aprovar.
- [ ] **MCPs autorizados**: Supabase MCP (DDL, migrations, edge functions, secrets) e Cloudflare MCP (Pages). MCP do Supabase **não cobre** Auth/SMTP/Templates — esses ajustes são manuais via Dashboard.
...
### 11.8. Passo 7 — Configurar Resend SMTP no Supabase Auth

Detalhes completos em `docs/SETUP_RESEND_SMTP.md`. Resumo:

1. **Auth → Providers → SMTP Settings** (toggle Enable Custom SMTP).
2. Preencher:
   ...
   - Port: `465`
   - Username: `resend`
   - Password: `RESEND_API_KEY` do vault
   - Minimum interval: `60` segundos
3. **Auth → Providers → Email**: toggle **Confirm email** ON, **Allow signups** ON.
...
- Email com OTP de 6 dígitos chega no inbox (Resend).
- `verifyOtp({ email, token, type: 'signup' })` popula `email_confirmed_at` e retorna sessão.
- Login pré-confirmação retorna `email_not_confirmed`; pós-confirmação retorna JWT.
...
- Acesso admin ao Resend Dashboard (gerenciar domínios e API keys)
- Acesso ao painel DNS da GoDaddy para a zona `plexalabs.com`
...
- [ ] Resend SMTP configurado no Supabase Auth (`smtp.resend.com:465`, sender `Caixa Boti <noreply@plexalabs.com>`).
- [ ] "Confirm email" ON em Auth → Providers → Email.
- [ ] Templates pt-BR aplicados (Confirm signup, Reset Password, Magic Link, Change Email — todos com `{{ .Token }}`).
- [ ] Smoke test de signup + OTP + login passou (`docs/SMOKE_TEST_FASE_1B.md`).
...
### 25.4. "OTP não chega no email"

**Causas possíveis:**
- DNS do Resend (DKIM/SPF) ainda não propagou ou está mal configurado para `plexalabs.com`.
- Email caiu no spam (acontece com domínios novos sem reputação).
- Rate limit do Supabase (default 60s entre emails para o mesmo destinatário).
- Template "Confirm signup" ainda usa `{{ .ConfirmationURL }}` em vez de `{{ .Token }}` — nesse caso vem magic link em vez de código.
- SMTP mal configurado: senha errada, porta diferente, sender_email não verificado no Resend.

**Solução:**
- Conferir pasta de spam.
- Resend Dashboard → **Logs** filtrando por destinatário: confirmar tentativa registrada e status `delivered` ou erro específico.
- Resend Dashboard → **Domains** → `plexalabs.com`: todos os checks DKIM/SPF/MX devem estar verdes.
- Supabase Dashboard → **Authentication → Logs**: buscar `email_send_failed` ou `smtp_error`.
- Conferir template em **Auth → Email Templates → Confirm signup**: corpo deve conter `{{ .Token }}` e **NÃO** `{{ .ConfirmationURL }}`.
- Aguardar 60s antes de tentar reenvio (rate limit padrão).
- Se persistir: trocar SMTP port para 587 (STARTTLS) — alguns provedores corporativos bloqueiam 465.
...
- Email + senha + OTP via Resend é a porta de entrada. Confirmação de email obrigatória; senha mínima 8 chars + 1 letra + 1 número (configurável em Supabase Auth → Password requirements).
...
**Passos:**
1. Login email + senha no celular (conta confirmada via OTP no signup).
...
- **Email + senha + OTP via Supabase Auth (Resend)** por logins próprios sem confirmação. Aumenta superfície de ataque (sem confirmação) e atrito (gerência de senhas reinventada).
...
| **OTP** | Código de 6 dígitos enviado por email (Resend SMTP) para confirmar cadastro ou recuperar senha. Expira em 1 hora. Variável de template no Supabase: `{{ .Token }}`. |
| **Resend** | Provedor SMTP transacional usado pelo Supabase Auth para enviar emails. Domínio `plexalabs.com` verificado, sender `Caixa Boti <noreply@plexalabs.com>`. |
...
|---|---|---|---|---|---|---|
| UAT-001 | Login Web email + senha + OTP | abrir caixa-boti.plexalabs.com, criar conta, receber OTP de 6 dígitos por email (Resend), confirmar conta, fazer login com email/senha | redireciona ao dashboard com JWT válido; login antes da confirmação é bloqueado com mensagem clara | | | |
